...
def fill_survey(survey_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()  # This will ensure the response gets an ID
...
# @views.route('/organization/survey_responses/<int:survey_id>', methods=['GET'])
# @login_required
# def organization_survey_responses(survey_id):
#     survey = Survey.query.get_or_404(survey_id)
#     if current_user.role != 'organization' or survey.owner_id != current_user.id:
#         flash('You do not have access to this survey responses', 'danger')
#         return redirect(url_for('home'))
#     responses = SurveyResponse.query.filter_by(survey_id=survey_id).all()
#     return render_template('organization_survey_responses.html', survey=survey, responses=responses)

# @views.route('/admin-survey-responses', methods=['GET'])
# @login_required
...
#     return render_template('admin_survey_responses.html', responses=responses)
# Trasa do wyświetlania ankiet do zatwierdzenia
